                ...
                <div class="card border-0 shadow-sm rounded-4 overflow-hidden">
                    <div class="card-header bg-light border-0 py-3">
                        <div class="d-flex justify-content-between align-items-center">
                            <h5 class="mb-0 fw-bold text-gradient">Edit Service</h5>
                        ...
                        <form action="{{ route('services.update', $service->id) }}" method="POST">
                            ...
                            <div class="mb-4">
                                ...
                                <div class="input-group">
                                    <input type="text" class="form-control rounded-end-3 @error('color') is-invalid @enderror"
                                           id="color" name="color" value="{{ old('color', $service->color) }}" placeholder="e.g. ffbb2c" required>
                                    <input type="color" id="colorPicker" class="d-none" value="#{{ old('color', $service->color) }}">
                                </div>
                                @error('color')
                                <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                                <small class="text-muted">Enter hex color code without the #</small>
                            </div>
                            ...
                            <div class="mb-4">
                                ...
                                <div class="input-group">
                                    <input type="text" class="form-control rounded-end-3 @error('image') is-invalid @enderror"
                                           id="image" name="image" value="{{ old('image', $service->image) }}" placeholder="e.g. bi bi-eye" required>
                                </div>
                                @error('image')
                                <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            ...
                            <div class="mb-4">
                                ...
                                <input type="number" class="form-control rounded-3 @error('order') is-invalid @enderror"
                                       id="order" name="order" value="{{ old('order', $service->order) }}" required>
                                @error('order')
                                <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
